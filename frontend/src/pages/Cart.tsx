		...
		<div className="max-w-6xl mx-auto p-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
			...
			<div className="bg-white rounded-xl shadow-xl p-4 h-fit">
				...
				<div className="space-y-2 text-sm">
					...
					<div className="flex justify-between"><span>Shipping</span><span>{formatCurrency(totals.shipping)}</span></div>
					<div className="flex justify-between"><span>Tax</span><span>{formatCurrency(totals.tax)}</span></div>
					<div className="border-t pt-2 flex justify-between font-semibold"><span>Total</span><span>{formatCurrency(totals.total)}</span></div>
				</div>
				<Link to="/checkout" className="block mt-4 rounded-full bg-gradient-to-r from-orange-500 to-teal-500 text-white text-center px-4 py-3 font-semibold shadow-lg hover:from-orange-600 hover:to-teal-600 transition-all duration-200 hover:shadow-xl">Checkout</Link>
			</div>
		</div>
	);
}
